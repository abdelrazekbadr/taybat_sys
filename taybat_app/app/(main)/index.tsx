...
function computeMealAlert(
  ...
  const dayCount = userMeals.filter(
    (m) => m.meal_id === mealId && m.date === todayStr,
  ).length;
  ...
  if (meal.max_day_frequency != null && dayCount > meal.max_day_frequency) {
    return {
      alertNote: 'لقد تجاوزت الحد الموصى به خلال اليوم',
      isOverLimit: true,
    };
  }
  if (meal.max_week_frequency != null && weekCount > meal.max_week_frequency) {
    return {
      alertNote: 'لقد تجاوزت الحد الموصى به خلال الأسبوع',
      isOverLimit: true,
    };
  }
  if (
    meal.max_month_frequency != null &&
    monthCount > meal.max_month_frequency
  ) {
    return {
      alertNote: 'لقد تجاوزت الحد الموصى به خلال الشهر',
      isOverLimit: true,
    };
  }
...
export default function HomeScreen() {
  ...
  // Always call initialize — the store decides (fresh TTL cache / offline cache /
  // network fetch), so a warm persisted cache still revalidates when online.
  useEffect(() => {
    if (isGuest) return;
    initializeUserMeals();
  }, [isGuest, initializeUserMeals]);

  // Recompute today's meals whenever the persisted cache hydrates or changes,
  // so the offline cold-start shows today's list without waiting for a focus event.
  useEffect(() => {
    if (isGuest) return;
    refreshTodayMeals();
  }, [isGuest, userMeals, refreshTodayMeals]);
  ...
  return (
    <View className="flex-1 bg-app-background">
      <HomeHeader
        ...
        onBellPress={
          isGuest ? requireLogin : () => router.push('/(main)/notifications')
        }
            ...
            {todayMeals.length > 0 && (
              <View className="mt-[22px]">
                ...
                >
                  <AppText
                    variant="bold"
                    className="text-[17px]  text-app-navy"
                  >
                    وجبات اليوم
                  </AppText>
                  <AppText
                    ...
                    const initialTab =
                      slotIndex === 0
                        ...
                        : slotIndex === 1
                          ? 'lunch'
                          : 'dinner';
                    const { alertNote, isOverLimit } = computeMealAlert(
                      meal,
                      userMeals,
                      todayStr,
                    );
                      ...
                      <TodayMealRow
                        ...
                        onPress={() =>
                          router.push({
                            ...
                            params: {
                              mealId: String(um.meal_id),
                              userMealId: String(um.id),
                            },
                          })
                        }
                        onReplacePress={() =>
                          router.push({
                            ...
                            params: {
                              replaceUserMealId: String(um.id),
                              initialTab,
                            },
                          })
                        }
                        onDeletePress={() => {
                          Alert.alert(
                            'حذف الوجبة',
                            'هل تريد حذف هذه الوجبة من سجل اليوم؟',
                            [
                              { text: 'إلغاء', style: 'cancel' },
                              {
                                ...
                                onPress: async () => {
                                  ...
                                  if (!ok) {
                                    const message =
                                      useUserMealsStore.getState().errorMessage;
                                    Alert.alert(
                                      'تعذّر حذف الوجبة',
                ...
                isGuest
                  ...
                  : () =>
                      router.push({
                        pathname: '/(main)/stats',
                        params: {
                          initialTab: pendingRating ? 'evaluation' : 'timeline',
                        },
                      } as never)
              }
            ...
            >
              ...
              >
                ...
                <View className="flex-1">
                  <AppText
                    variant="bold"
                    className="text-[14px] leading-6 text-app-navy"
                  >
                    إنجازاتي
                  </AppText>
            ...
            {!isGuest && !displayUser.profile_completed && (
              ...
              >
                <View className="flex-row" style={{ flexDirection: rowDir }}>
                  ...
                  <View
                    className="flex-1 items-center gap-3 px-4 py-3.5"
                    style={{ flexDirection: rowDir }}
                  >
            ...
            <View className="mt-[22px]">
              <WeeklyProgressBar
                ...
                onPress={
                  isGuest
                    ? requireLogin
                    : () => router.push('/(main)/meal-history' as never)
                }
              />
            </View>
          </View>
