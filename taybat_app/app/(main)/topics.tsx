    ...
    <View className="flex-1 bg-app-background">
      ...
      >
        ...
        <View className="items-center gap-0.5">
          <AppText variant="bold" className="text-center text-[17px]  text-app-navy">
            المكتبة
          </AppText>
        </View>
        <View className="w-10" />
      ...
      {isLoading && topics.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item }: { item: LibraryTopicWithItems }) => {
            const accentColor = resolveAccentColor(theme, item.code, item.accent_color);
            ...
            return (
              ...
              >
                ...
                <View className="flex-1 bg-app-surfaceAlt px-3.5 py-3">
                  <AppText variant="bold" className="text-[14.5px] leading-6 text-app-navy" numberOfLines={2}>
                    {item.title}
                  </AppText>
                  <AppText className="mt-1 text-[11.5px] leading-6 text-app-textSoft" numberOfLines={2}>
                    {item.description}
                  </AppText>
                </View>
              </TouchableOpacity>
